...
func TestReconcile(t *testing.T) {
	...
	cases := map[string]struct {
		...
		"reconcile DRA ResourceClaimTemplate rejected when DRA disabled and DRARejectWorkloadsWhenDRADisabled enabled": {
			featureGates: map[featuregate.Feature]bool{
				features.DynamicResourceAllocation:         false,
				features.DRARejectWorkloadsWhenDRADisabled: true,
				features.MultiKueueOrchestratedPreemption:  false,
			},
			...
			wantWorkload: utiltestingapi.MakeWorkload("wlWithDRAResourceClaimTemplate", "ns").
				...
				Condition(metav1.Condition{
					...
					Status:  metav1.ConditionFalse,
					Reason:  kueue.WorkloadInadmissible,
					Message: "Workload uses DRA resources but the DynamicResourceAllocation feature gate is not enabled",
				}).
				Condition(metav1.Condition{
					Type:    kueue.WorkloadRequeued,
					Status:  metav1.ConditionFalse,
		...
		"reconcile DRA ResourceClaim rejected when DRA disabled and DRARejectWorkloadsWhenDRADisabled enabled": {
			featureGates: map[featuregate.Feature]bool{
				features.DynamicResourceAllocation:         false,
				features.DRARejectWorkloadsWhenDRADisabled: true,
				features.MultiKueueOrchestratedPreemption:  false,
