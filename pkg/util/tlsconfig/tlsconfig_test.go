...
func TestBuildTLSOptions(t *testing.T) {
	tests := []struct {
		...
		{
			name: "invalid TLS version defaults to 1.2",
			cfg: &config.TLSOptions{
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if !cmp.Equal(testConfig.CurvePreferences, tt.expectedCurves) {
				t.Errorf("CurvePreferences diff: %s", cmp.Diff(tt.expectedCurves, testConfig.CurvePreferences))
			}
		})
	}
}
