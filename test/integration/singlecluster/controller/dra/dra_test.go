...
var _ = ginkgo.Describe("DRA Integration", ginkgo.Ordered, ginkgo.ContinueOnFailure, func() {
	...
	ginkgo.When("DynamicResourceAllocation disabled with DRARejectWorkloadsWhenDRADisabled enabled", func() {
		var (
			ns             *corev1.Namespace
			resourceFlavor *kueue.ResourceFlavor
			clusterQueue   *kueue.ClusterQueue
		...
		ginkgo.BeforeEach(func() {
			...
			features.SetFeatureGateDuringTest(ginkgo.GinkgoTB(), features.DRARejectWorkloadsWhenDRADisabled, true)
			ns = &corev1.Namespace{
				ObjectMeta: metav1.ObjectMeta{
		...
		ginkgo.It("Should reject workload with ResourceClaimTemplate when DRA is disabled", func() {
			...
			gomega.Eventually(func(g gomega.Gomega) {
				...
				g.Expect(updatedWl.Status.Conditions).To(gomega.ContainElement(gomega.And(
					...
					gomega.HaveField("Status", metav1.ConditionFalse),
					gomega.HaveField("Reason", kueue.WorkloadInadmissible),
					gomega.HaveField("Message", gomega.ContainSubstring("DynamicResourceAllocation feature gate is not enabled")),
				)))
			}, util.Timeout, util.Interval).Should(gomega.Succeed())
		})
