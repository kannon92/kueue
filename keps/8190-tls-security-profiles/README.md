...
- **CipherSuites**: If not specified, Go's default secure cipher suite selection will be used
  - For TLS 1.2, recommended suites include:
...
- **CurvePreferences**: If not specified, Go's default curve preferences will be used
  - Recommended: X25519, CurveP256, CurveP384
...
func buildTLSConfig(cfg *config.TLSOptions) ([]func(*tls.Config), error) {
	...
	tlsOpts = append(tlsOpts, func(c *tls.Config) {
		...
		switch strings.ToUpper(cfg.MinTLSVersion) {
		case "1.2", "":
		...
		if len(cfg.CurvePreferences) > 0 {
			c.CurvePreferences = convertCurvePreferences(cfg.CurvePreferences)
		}
	})
