...
unlimited GPU consumption outside Kueue's control. The `DRARejectWorkloadsWhenDRADisabled` feature gate
...
- `DRARejectWorkloadsWhenDRADisabled` (default: enabled, Beta since v0.18): rejects workloads that
...
The `DRARejectWorkloadsWhenDRADisabled` feature gate (default: enabled, Beta) closes this gap. When
...
management can disable `DRARejectWorkloadsWhenDRADisabled` to restore the previous behavior.
...
- DRA disabled rejection: Testing that workloads with ResourceClaimTemplates or ResourceClaims are
  rejected as inadmissible when `DynamicResourceAllocation` is off and `DRARejectWorkloadsWhenDRADisabled` is on,
...
- `DRARejectWorkloadsWhenDRADisabled` feature gate added: May 2026 by @kannon92 — rejects DRA workloads
